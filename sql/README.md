...
| 014 | `014_re_rank_all_ladders.sql` | Re-ranks all ladders globally, compacting gaps. **Actually applied 2026-09-24** — it had sat unapplied since 2026-09-16 (marked "Needs manual apply"), which is exactly why P Karthik Squash Institute's ladder was found ranked `2,3,4,5,6` (missing #1) on 2026-09-24. Ran it: all 7 ladders now `1..N`, confirmed via API and the live club page. | ✅ Applied 2026-09-24 |
| 030 | `030_auto_rerank_on_delete.sql` | **Prevents recurrence**: an `AFTER DELETE` trigger on `ladder_players` (`rerank_ladder_after_delete()`) that automatically compacts a ladder's ranks back to `1..N` the moment a player is removed, no matter which code path deletes the row (admin panel, a player leaving, future features). SECURITY DEFINER, idempotent. | ✅ Applied 2026-09-24. Verified: trigger exists and enabled (`tgenabled = 'O'`), all ladders still correctly ranked afterward, all public pages 200 |
