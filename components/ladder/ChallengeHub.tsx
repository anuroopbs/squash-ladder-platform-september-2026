...
export function ChallengeHub({ ladderId, standings }: { ladderId: string; standings: LadderStanding[] }) {
  ...
  // Native <select> dropdown popups are rendered by the browser/OS, not by
  // our page CSS — the closed box picks up our dark styling, but individual
  // <option> rows fall back to the browser's own default (usually a white
  // popup) unless we give them an explicit background too. Without this,
  // the white text color inherited from the page becomes invisible white-on-
  // white, except on the one row the browser happens to highlight on hover.
      ...
      h(
        ...
        h(
          ...
          h(
            ...
            h("option", { value: "", className: optionClass }, "Choose an opponent…"),
            opponents.map((o) =>
              h("option", { key: o.player_id, value: o.player_id, className: optionClass }, o.display_name)
            )
          ),
          h(
            "button",
              ...
              reportingId === c.id &&
                h(
                  ...
                  h(
                    ...
                    h("option", { value: c.challenger_id, className: optionClass }, `${nameOf(c.challenger_id)} won`),
                    h("option", { value: c.challenged_id, className: optionClass }, `${nameOf(c.challenged_id)} won`)
                  ),
                  h("input", {
                    type: "text",
                    value: score,
